...
function calculateMoonPhase(date) {
    ...
    // Calculate illumination: more accurate using sine curve approximation
    const illumination = (1 - Math.cos(2 * Math.PI * phase)) / 2;

    return {
        icon: currentPhase.icon,
        name: currentPhase.name,
        illumination: (illumination * 100).toFixed(1) // percentage with 1 decimal place
    };
}
...
function updateMoonPhase() {
    const date = new Date();
    const moonData = calculateMoonPhase(date); // must return { icon, name, illumination }
...
async function fetchWindData() {
    try {
        ...
        if (data.wind) {
            ...
            const windGust = data.wind.gust || null; // gust may not always be present

            // Convert m/s to km/h
            const windSpeedKmh = (windSpeed * 3.6).toFixed(1);
            const windGustKmh = windGust ? (windGust * 3.6).toFixed(1) : null;

            // Update wind speed
            document.getElementById("wind-speed").textContent = `${windSpeedKmh} km/h`;
 document.getElementById("compass-wind-speed").textContent = `${windSpeedKmh}`;

            // Update gust if available
            if (windGustKmh) {
                document.getElementById("wind-gust").textContent = `Gusts ${windGustKmh} km/h`;
            } else {
                document.getElementById("wind-gust").textContent = "No gust data";
            }
...
document.getElementById("compass-wind-speed").textContent = "--";
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    setInterval(fetchRainData, 900000); // 15 minutes
        ...
        function getWeekDays() {
            ...
            // Return today + next 6 days
            return reorderedDays.slice(0, 7);
        }

        // Sample forecast data (replace with real API data)
        function generateForecastData() {
            const dayNames = getWeekDays();
            const icons = ['sun', 'cloud-sun', 'cloud', 'cloud-rain', 'cloud-showers-heavy', 'cloud-sun-rain', 'bolt'];
            const rainAmounts = [0, 0.2, 0.8, 1.2, 2.4, 3.1, 4.5];
            
            return dayNames.map((day, index) => ({
                day: index === 0 ? 'Today' : day.slice(0, 3),
                rain: rainAmounts[index],
                icon: icons[index]
            }));
        }
        ...
        function createForecastCards() {
            ...
            forecastData.forEach((day, index) => {
                ...
                let iconClass = '';
                if (day.icon.includes('sun')) iconClass = 'sun-icon';
                else if (day.icon.includes('rain') || day.icon.includes('shower')) iconClass = 'rain-icon';
                else iconClass = 'cloud-icon';
                
                card.innerHTML = `
                    <div class="forecast-day">${day.day}</div>
                    <div class="weather-icon">
                        <i class="fas fa-${day.icon} ${iconClass}"></i>
                    </div>
        ...
        function showForecast() {
            ...
            mainDashboard.style.transform = 'translateY(-100%)';
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Initial delay before showing forecast
            setTimeout(showForecast, 40000);
        });